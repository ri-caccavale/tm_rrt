...
getGoalState:-
	yield([],_),
	yield([free(p5),free(p6),free(p7),free(p3),on(c1,p1),on(c2,p2),on(c3,p8),on(c4,p4)],_),!.

getInitialState:-
	yield([],_),
	yield([free(p5),free(p6),free(p7),free(p8),on(c1,p1),on(c2,p2),on(c3,p3),on(c4,p4)],_),!.

getMap:-
	yield([],_),
	yield(["/maps/map_anomaly2_X.png"],_),!.
...
object(c4,[
	shape(-0.625,-0.47,0.0,0.15,0.1,0.01),
  	shape( 0.625,-0.47,0.0,0.15,0.1,0.01),
  	shape(-0.625, 0.47,0.0,0.15,0.1,0.01),
  	shape( 0.625, 0.47,0.0,0.15,0.1,0.01) ]). %% c4 and p4 are only "distractions"
