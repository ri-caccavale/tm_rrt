...
TM_RRTplanner::TM_RRTplanner(std::string path_to_node_directory, std::string domain_file_name) : random_generator(std::random_device()()) {
    ...
    eclipse = new Eclipseclp_interface(path_to_node_directory + "/src/eclipseclp_interface/eclipseclp");
    eclipse->open(path_to_node_directory + "/" + domain_file_name);
...
void TM_RRTplanner::domain_from_PROLOG() {
    ...
    for (int i = 0; i < objects.size(); i++) {
        ...
        for (int j = 0; j < shapes.size(); j++) {
            ...
            std::vector<std::string> shape_vec = instance2vector(shapes[j]);

            obj.addShapeRect(std::stof(shape_vec[1]),
                             std::stof(shape_vec[2]),
                             std::stof(shape_vec[3]),
                             std::stof(shape_vec[4]),
                             std::stof(shape_vec[5]));
    ...
    for (int i = 0; i < poses.size(); i++) {
        ...
        std::vector<std::string> pose_vec = instance2vector(poses[i]);
        //store the pose into the pose_map to be retreived during planning
        pose_map[pose_vec[1]] = Pose3d(std::stof(pose_vec[2]),
                                       std::stof(pose_vec[3]),
                                       std::stof(pose_vec[4]) );
    }
...
Pose3d TM_RRTplanner::find_pose(std::unordered_map<std::string, bool>& var, std::string toFind) {
    ...
    for (auto it : var) {
        //parse the predicate 
        varVector = instance2vector(it.first);
