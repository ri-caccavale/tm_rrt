%
%PLANNING DOMAIN FOR 3-CARTS TASK_AND_MOTION 
%
...
getGoalState:-
	yield([],_),
	yield([free(p1),free(p2),free(p3),free(p4),
        	on(c1,p5),on(c2,p6),on(c3,p7),on(c4,p8)],_),!.

getInitialState:-
	yield([],_),
	yield([free(p5),free(p6),free(p7),free(p8),
		on(c1,p1),on(c2,p2),on(c3,p3),on(c4,p4)],_),!.
